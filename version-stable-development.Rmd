...
[Rocker project](https://www.rocker-project.org/), and allowing  the coexistence
...
versions (e.g. `rocker/r-ver:3.5.1`). Besides being specific to the
...
Assume we want to run a containerized versioned instance of RStudio for R 3.5.1,
...
- Use a version-specific port, e.g. `3510` for R 3.5.1, `3600` for R 3.6.0 and
...
`~/.rstudio-docker/3.5.1`) for the `home/rstudio/.rstudio` directory, which is
version-specific in case of multiple R versions
- If we want to use Meld via the [compareWith](https://github.com/miraisolutions/compareWith/) addins (see [rstudio-compareWith.md](rstudio-compareWith.md) for details), we need to
...
```
The running RStudio can then be accessed by visiting `http://localhost:3510/`.

You may find convenient to define an alias as follows
...
run_rstudio_ver 3.5.1 GitHubProjects
```
...
a version-stable TeXLive repository is used for the TinyTeX install (see
...
`rocker/verse:3.5.1`, since it was build at the end of 2018 but the current
TeXLive repo is 2019. You can fix this via a **user-specific** re-installation of
...
```{bash check-path}
docker exec --user rstudio rstudio_3.5.1 R --slave -e 'Sys.getenv("PATH")'
```
If you don't see `/home/rstudio/bin`, you can make sure it is part of the `PATH` for R via
```{bash set-path}
docker exec --user rstudio rstudio_3.5.1 sh -c 'echo "PATH=$HOME/bin:\${PATH}" >> $HOME/.Renviron'
# check again
docker exec --user rstudio rstudio_3.5.1 R --slave -e 'Sys.getenv("PATH")'
```
...
```{bash cleanup}
docker rm $(docker stop rstudio_3.5.1)
```
