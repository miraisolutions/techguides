...
`podman` can be used instead of `docker` to run the above commands. In that case, the active user will be _root_, 
which is then mapped to the user id of the user running the container when writing files to the shared volume.
...
If you run into issues, this rocker [pull request](https://github.com/rocker-org/rocker-versioned2/pull/636) 
