...
- Deploy to shinyapps.io:
  - In order to provide credentials for the deployment, account name and corresponding [tokens](https://docs.rstudio.com/shinyapps.io/getting-started.html#deploying-applications) for shinyapps.io, environment variables `SHINYAPPS_ACCOUNT`, `SHINYAPPS_TOKEN` and `SHINYAPPS_SECRET` are specified / accessible as GitHub [secrets](https://docs.github.com/en/free-pro-team@latest/actions/reference#authentication-and-secrets).
...
As visible from the run logs that can be found in the GitHub repository under the `Actions` tab, all the CI/CD pipeline steps are performed subsequently, and are identifiable by the `name` field. See the example below:
...
In order to setup and use CI/CD GitHub Actions workflows as described above, we provide full YAML workflow files for CI and CI/CD pipelines with and without `renv`, which you can simply include in your project via:
```{r}
usethis::use_github_action(
  url = 
