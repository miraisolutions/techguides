...
This guide illustrates how to set up CI/CD pipelines on the new and increasingly popular [GitHub Actions](https://github.com/features/actions) and the previously fully open source [Travis CI](https://travis-ci.com). It also shows how a packaged Shiny app can be maintained on a GitHub repository, and deployed and hosted on [shinyapps.io](https://www.shinyapps.io) using said CI/CD pipelines.
...
A workflow should have an identifying `name` and an `on` section that indicates upon which events the workflow should be triggered. It should include at least one job and each job will have a set of steps fully specifying what to execute. Such steps can be a (predefined) action or a script, however, for the time being, GitHub Actions does not support a hierarchical, aggregated structure of actions. Being fully customizable, it is necessary to fully specify each step in the CI/CD pipeline explicitly.
