# Control Microsoft Excel interface with `XLConnect`
...
- specify/ use cell styles, column width & row height

- define behavior when error cells are encountered, formula recalculation when workbooks are opened

- others: merge/unmerge cells, set auto-filters etc.
...
# Load XLConnect and get ready to use XLConnect package
require(XLConnect)
```
...
The data frame can also be written in a named region using the single call `writeNamedRegionToFile()` function.
```{r , echo = TRUE, eval = FALSE}
